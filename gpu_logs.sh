...
printf "%-8s | %-8s | %-15s | %-12s | %-15s\n" "GPU ID" "PID" "PROCESS" "VRAM" "ELAPSED TIME"
printf "%s\n" "------------------------------------------------------------------------"
...
nvidia-smi --query-compute-apps=gpu_uuid,pid,process_name,used_memory --format=csv,noheader | while IFS=, read -r uuid pid name mem; do
    ...
    # Print the formatted row with the new GPU ID
    printf "GPU %-4s | %-8s | %-15s | %-12s | %-15s\n" "$gpu_id" "$pid" "$name" "$mem" "$etime"
done
