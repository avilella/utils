...
def main():
    ...
    if args.verbose:
        print(f"[INFO] Fetching recent posts for {len(candidates)} candidates...", file=sys.stderr)
    ...
    for idx, row in enumerate(candidates):
        ...
        if args.verbose and (idx + 1) % 50 == 0:
            print(f"       ... fetched {idx + 1}/{len(candidates)} candidates", file=sys.stderr)

    if not valid_candidates:
        print("Error: Could not extract any post text from the candidate accounts.", file=sys.stderr)
        sys.exit(1)
